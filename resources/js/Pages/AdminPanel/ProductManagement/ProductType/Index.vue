                            ...
                            <div class="card">
                                ...
                                <div class="card-body table-responsive p-0">
                                    <table class="table table-hover text-nowrap">
                                        ...
                                        <tbody>
                                            <tr v-for="(product_type, index) in product_types.data" :key="index">
                                                <td>
                                                    <img class="w-10 h-10 rounded-full" :src="product_type.image" alt="" style="width:60px;"/>
                                                    {{ product_type.name }}
                                                </td>
                                                <td>{{ product_type.key }}</td>
                                                <td>{{ dateTime(product_type.created_at) }}</td>
                                                <td class="text-right" v-if="$page.props.auth.hasRole.superAdmin || $page.props.auth.hasRole.admin">
                                                    <button class="btn btn-success text-uppercase" style="letter-spacing: 0.1em;" @click="editModal(product_type)">Edit</button>
                                                    <button class="btn btn-danger text-uppercase ml-1" style="letter-spacing: 0.1em;" @click="deleteProductType(product_type.id)">Delete</button>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                ...
                                <div class="card-footer clearfix">
                                    <pagination :links="product_types.links"></pagination>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            checkMode() {
                return this.editMode === false ? this.createProductType : this.editProductType
            }
        },
        watch: {
            params: {
        ...
        methods: {
            ...
            editModal(product_type) {
                ...
                this.form.key = product_type.key
            },
            openModal() {
                this.editedIndex = -1
                $('#modal-lg').modal('show')
            ...
            createProductType() {
                this.form.post(this.route('admin.product_types.store'), {
                    preserveScroll: true,
                    onSuccess:() => {
                        this.closeModal()
                        Toast.fire({
